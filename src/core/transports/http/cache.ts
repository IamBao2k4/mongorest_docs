...
export class CacheRoutes {
  ...
  private async handleCacheDelete(
    action: string,
    collection: string | undefined,
    res: http.ServerResponse
  ): Promise<void> {
      ...
      case "invalidate":
        if (!collection) {
          this.sendError(
            res,
            400,
            "Collection name required for invalidate action"
          );
          return;
        }
        await this.dbAdapter.clearCache(collection);
  ...
  private async handleCachePost(
    ...
    let jwt =
      "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJ1c2VyX2RlZmF1bHRfMDAxIiwidXNlcklkIjoidXNlcl9kZWZhdWx0XzAwMSIsInVzZXJuYW1lIjoiZ3Vlc3RfdXNlciIsInJvbGVzIjoiZGVmYXVsdCIsImlzQWRtaW4iOmZhbHNlfQ.p21cymLG1Q-flME3vyB84TP1Whd1zqQOmhAbWA3bjPs";
      ...
      case "warmup":
        if (!collection) {
          this.sendError(
            res,
            400,
            "Collection name required for warmup action"
          );
          return;
        }
