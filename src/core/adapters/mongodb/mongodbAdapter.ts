...
export class MongoDBAdapter extends BaseDatabaseAdapter {
  ...
  constructor(relationshipRegistry?: RelationshipRegistry, entitiesFilePath?: string) {
    ...
    this.entityManager = new EntityManager(entitiesFilePath);
  ...
  // Delegate methods to EntityManager
  getEntities() {
    return this.entityManager.getEntities();
  }

  getEntityByCollectionName(collectionName: string) {
    return this.entityManager.getEntityByCollectionName(collectionName);
  }

  getCollectionNames() {
    return this.entityManager.getCollectionNames();
  }

  reloadEntities() {
    this.entityManager.reloadEntities();
  }

  // Additional delegated methods
  async getCollection(collectionName: string) {
    return this.entityManager.getCollection(collectionName);
  }

  async createCollection(collectionName: string) {
    return this.entityManager.createCollection(collectionName);
  }

  async dropCollection(collectionName: string) {
    return this.entityManager.dropCollection(collectionName);
  }

  async createCollectionWithSchema(collectionName: string, schema: any) {
    return this.entityManager.createCollectionWithSchema(collectionName, schema);
  }

  async updateCollectionSchema(collectionName: string, schema: any) {
    return this.entityManager.updateCollectionSchema(collectionName, schema);
  }

  async listCollections() {
    return this.entityManager.listCollections();
  }

  async watchCollection(collectionName: string, targetCollectionName: string, type: string) {
    return this.entityManager.watchCollection(collectionName, targetCollectionName, type);
  }

  async getEntityLocal() {
    return this.entityManager.getEntityLocal();
  }
}