...
class EntityService {
    ...
    async createEntityFile(entity: any) {
        ...
        let _entity = await this.findOne(entity.collection_name);
        if (_entity) {
            throw new Error(`Entity with collection name ${entity.collection_name} already exists`);
        }
        let dirPath = path.join(process.cwd(), 'json', 'entities');
        if (!fs.existsSync(dirPath)) {
            fs.mkdirSync(dirPath, { recursive: true });
        }
        let fileName = `${entity.collection_name}.json`;
